...
import type { ActiveRoundData } from '../types/game';
...
export const ResultsScreen = ({
  ...
  const eliminatedPlayer = roundData.players.find(p => p.id === roundData.eliminatedPlayerId);
  const isImposterCaught = eliminatedPlayer && eliminatedPlayer.role === 'imposter';

  const civiliansWin = roundData.winner === 'civilians';
      ...
      <div className={`p-6 rounded-3xl border-2 text-center shadow-2xl relative overflow-hidden mb-6 ${
        ...
        <div className="mt-4 pt-4 border-t border-white/10 flex flex-col items-center">
          ...
          <span className="text-2xl font-extrabold text-amber-300 mt-0.5">
            "{roundData.civilianWord}"
          </span>
          <span className="text-[11px] text-slate-400 mt-1">
            Category: {roundData.categoryName}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 mb-6">
        ...
        <div className="flex flex-col gap-2">
          {roundData.players.map((p) => {
            ...
            const isElim = p.id === roundData.eliminatedPlayerId;
      ...
      <div className="fixed bottom-4 left-0 right-0 max-w-md mx-auto px-4 flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-amber-400 font-bold text-xs hover:bg-slate-700 transition flex items-center justify-center gap-1.5"
          >
          ...
          <button
            onClick={onGoHome}
            className="py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-300 font-bold text-xs hover:bg-slate-700 transition flex items-center justify-center gap-1.5"
          >
